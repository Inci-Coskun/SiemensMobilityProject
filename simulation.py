...
class PiezoEnergyPredictor:
    ...
    def predict_power_from_footfall(self, footfall, avg_weight=65):
        ...
        # FIX: The previous code had a hardcoded minimum of 0.1 W, which caused the "glitch."
        # The new code now removes this artificial minimum, allowing generation to drop to 0.
...
def ai_models_page():
    """AI models page."""
    st.header("🤖 AI Models & Performance")
    st.write(
        "This page provides a detailed look at the machine learning models that power the simulation. "
        "The models were pre-trained on the embedded datasets to predict energy generation and system consumption."
    )
    
    st.markdown("---")
    
    st.subheader("Piezoelectric Energy Predictor (Linear Regression)")
    st.markdown("""
        This model predicts the power generated by piezoelectric tiles based on footfall characteristics
        (voltage, current, weight, and step location).
        * **Model Type:** Linear Regression
        * **Purpose:** To estimate real-time energy generation from pedestrian traffic.
    """)
    if "piezo_mae" in st.session_state and "piezo_r2" in st.session_state:
        st.info(f"""
            **Model Performance Metrics:**
            * **Mean Absolute Error (MAE):** {st.session_state.piezo_mae:.3f}
            * **R-squared (R²):** {st.session_state.piezo_r2:.3f}
        """)
        
    st.markdown("---")

    st.subheader("System Consumption Predictor (Random Forest Regressor)")
    st.markdown("""
        This model predicts the power draw of various station systems (CCTV, panels, etc.) 
        based on time, footfall, ambient temperature, and day type.
        * **Model Type:** Random Forest Regressor
        * **Purpose:** To predict the energy demand for optimal allocation.
    """)
    st.info("The individual models for each system (e.g., CCTV, Ventilation) are trained on a synthetic dataset.")
    
    st.markdown("---")

    st.subheader("Smart Allocation Agent")
    st.markdown("""
        This agent is the brain of the micro-grid. It takes the predicted energy generation and consumption
        and intelligently allocates power based on a pre-defined priority list.
        * **Strategy:** Prioritizes "Critical" systems (like CCTV) over "High" and "Medium" priority systems.
        * **Adaptation:** Dynamically adjusts to power shortages by providing partial power or shutting off low-priority systems.
    """)

def simulation_page():
    """Simulation page where the real-time control loop runs."""
    local_css()
    
    st.title("⚡ AI Micro-Grid Simulation")
    st.write("Welcome to the real-time simulation of a smart energy micro-grid.")
    
    # --- UI Controls ---
    st.sidebar.title("Control Panel")
    start_button = st.sidebar.button("▶️ Start Simulation")
    pause_button = st.sidebar.button("⏸️ Pause Simulation")
    ...
    if "battery_energy" not in st.session_state:
        st.session_state.battery_energy = 50.0 # Initial battery charge in watts
    ...
    # Display current time and next event
    current_time_placeholder.markdown(f"**Current Simulated Time:** {st.session_state.simulated_time.strftime('%H:%M:%S on %A, %B %d')}")
    next_event_placeholder.markdown(f"**Next Event:** {get_next_event(st.session_state.simulated_time)}")
